...
def get_changelog_text(lang_name: str = "English") -> str:
    ...
    if lang_name in ("Polski", "Polish"):
        return (
            ...
            "• v1.3.8 - v1.2.5 (Kumulacyjna Aktualizacja Stabilności i Działania na macOS):\n"
            "  - Całkowicie wymieniono implementację ekstrakcji cech dźwiękowych (MFCC) na własną opartą wyłącznie o NumPy. Rozwiązuje to całkowicie problemy na macOS wynikające z `scipy`, które powodowały crashe 'dyld: __DATA/__thread_bss zero-fill'.\n"
            "  - Zaktualizowano budowanie aplikacji na macOS za pomocą PyInstallera by unikać psujących binarki stripów.\n"
            "  - Przeniesiono pliki logów do ~/Library/Logs/Focus aby obejść restrykcje TCC na macOS.\n"
            "  - Dodano system auto-unquarantine (odblokowania przez Gatekeeper) przy pierwszym uruchomieniu na Macu.\n"
            "  - Poprawki interfejsu (obcięte teksty, znak ampersand).\n\n"
        ...
        return (
            ...
            "• v1.3.8 - v1.2.5 (Cumulative macOS Stability & Pure NumPy MFCC Rewrite):\n"
            "  - Completely replaced `scipy` and `python_speech_features` dependencies with a pure NumPy MFCC extraction implementation. This resolves the persistent `dyld: __DATA/__thread_bss` zero-fill macOS crashes caused by PyInstaller stripping the SciPy Fortran binaries.\n"
            "  - Fixed PyInstaller macOS zipping process (`zip -ry`) to preserve symlinks in the app bundle.\n"
            "  - Bypassed macOS TCC restrictions by relocating persistent logs to `~/Library/Logs/Focus`.\n"
            "  - Implemented silent Gatekeeper auto-unquarantining and zero-terminal macOS launching.\n"
            "  - Resolved UI layout clipping, ampersand mnemonics bugs, and applied ECC stability patches.\n\n"
            "• v1.2.1 (Audio Track Selector, Dynamic Aspect Ratio & Documents Log Location):\n"
